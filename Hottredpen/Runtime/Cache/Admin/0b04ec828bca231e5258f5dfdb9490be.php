...
<div class="container">
	...
	<div id="content" class="col-md-10">
		<ul class="breadcrumb">
			...
			<li class="active">修改<?php echo ($article["modname"]); ?>Tags</li>
		</ul>
            ...
            <div id="myTabContent" class="tab-content">
                <form method="post"  action="__URL__/updatealltags">
							...
							<table class="table">
											<tbody>
											  ...
											  <tr>
												...
												<td>
												  <select name="pid">
												  <?php if(!empty($pid)): if(is_array($pdata)): $i = 0; $__LIST__ = $pdata;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($vo["id"] == $pid): ?><option selected="selected" value="<?php echo ($vo["id"]); ?>" class="cal<?php echo ($vo["order"]); ?>">
																   ...
																   <?php else: ?>
																		<option value="<?php echo ($vo["id"]); ?>" class="cal<?php echo ($vo["order"]); ?>">
																			<?php if(intval($vo['order']) == 1 ): echo ($vo["title"]); ?>
																			<?php elseif(intval($vo['order']) == 2 ): ?>
																				├ <?php echo ($vo["title"]); ?>
																			<?php elseif(intval($vo['order']) == 3 ): ?> 
													   ...
													   <?php else: ?>
														<option>请给他分类</option>
												       <?php if(is_array($pdata)): $i = 0; $__LIST__ = $pdata;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" class="cal<?php echo ($vo["order"]); ?>">
																<?php if(intval($vo['order']) == 1 ): echo ($vo["title"]); ?>
																<?php elseif(intval($vo['order']) == 2 ): ?>
																	├ <?php echo ($vo["title"]); ?>
																<?php elseif(intval($vo['order']) == 3 ): ?> 
