...
class Cutimages{
    ...
    function thumb($image,$is_save=true,$suofang=0,$type='jpg',$maxWidth=500,$maxHeight=500,$interlace=true){
        // 获取原图信息
       $info  = $this->getImageInfo($image);
         if($info !== false) {
			...
			$_type = substr($image,-3);
			$length = ($type != $_type ? $length+1 : $length);
            //裁剪
            if ($suofang==1) {
				
				$thumbname01 = substr_replace($image,"01.".$type,$length+2);		//大头像
				$thumbname02 = substr_replace($image,"02.".$type,$length+2);		//小头像
				$imageFun($thumbImg,$thumbname01,100);
				$imageFun($thumbImg,$thumbname02,100);

                $thumbImg01 = imagecreatetruecolor(290,192);
                imagecopyresampled($thumbImg01,$thumbImg,0,0,$_POST['x'],$_POST['y'],290,192,$_POST['w'],$_POST['h']);

				$thumbImg02 = imagecreatetruecolor(145,96);
                imagecopyresampled($thumbImg02,$thumbImg,0,0,$_POST['x'],$_POST['y'],145,96,$_POST['w'],$_POST['h']);
