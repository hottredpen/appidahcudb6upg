...
class DbManage {
	...
	function backup($tablename = '', $dir, $size) {
		...
		if (! empty ( $tablename )) {
		   $tablenames=explode(',',$tablename);
			...
			// 插入dump信息
			$sql= $this->_retrieve ();
			foreach($tablenames as $id=>$tablename){
			// 插入表结构信息
	...
	private function _retrieve() {
		...
		$value .= '-- Created by DBManage class, Power By dswj. ' . $this->ds;
		$value .= '-- http://dianshiweijin.com ' . $this->ds;
		$value .= '--' . $this->ds;
	 ...
	 * @table			表名	
	 */
	function list_tables($database,$table='')  
    ...
    $tables = array();
    if($table){	//如果是单表信息
		$nature=mysql_query("SHOW COLUMNS FROM ".$table."");
		while( $r= mysql_fetch_array($nature) ){
			$tables[]=$r;
		}  
	}else{
		while ($row = mysql_fetch_row($rs)) {  
			$nature=mysql_query("SHOW TABLE STATUS from  $database  where Name='".$row[0]."'");
			while( $r= mysql_fetch_array($nature) ){
				$natures=$r;
			}  
			$tables[] = array('table'=>$row[0],'nature'=>$natures);
		}  
	}
